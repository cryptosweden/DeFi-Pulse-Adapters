...
const _run = async (projectAdapter, timeUnit = 'day', timeOffset = 0) => {
  try {
    ...
    const point = await sdk.api.util.lookupBlock(
      timestamp,
      projectAdapter.chain
    )
    ...
    if (projectAdapter.tvl && sdk.api.util.isCallable(projectAdapter.tvl)) {
      ...
      await Promise.all(
        projectAdapter.tokenHolderMap.map(async (thm) => {
    ...
    const tvl = await sdk.api.util.testAdapter(
      point.block,
      timestamp,
      projectAdapter,
      tokenBalanceMap
...
module.exports = async (projectAdapter, timeUnit, timeOffset = 0) => {
  ...
  if (typeof timeUnit === 'number') {
    label = `returns valid tvl data at ${moment
      .unix(timeUnit)
      .utcOffset(0)
      .format()}. ${labelExt}`
  } else {
  ...
  afterEach('save project adapter output', () => {
    let path
    path = `output/${projectAdapter.chain || 'ethereum'}/${
      projectAdapter.name
    }/tvl`
