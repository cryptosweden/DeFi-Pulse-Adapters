...
const categories = ['Derivatives', 'DEXes', 'Lending', 'Payments', 'Assets']
...
_.each(args.projects, (projectAdapter) => {
  describe(`Checking ${projectAdapter.name} project adapter metadata`, () => {
    ...
    it('has a valid start time', () => {
      chai
        .expect(projectAdapter.start)
        .to.be.at.most(
          moment().utcOffset(0).unix(),
          'unix start time must be less than the current time'
        )
    })
