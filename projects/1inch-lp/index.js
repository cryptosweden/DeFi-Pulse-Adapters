...
async function tvlByVersion (factory, startBlock, block) {
  const supportedTokens = await sdk.api.util
    ...
    .then((supportedTokens) =>
      supportedTokens.map(({ contract }) => contract.toLowerCase())
    )
  ...
  const pairs = logs.reduce((acc, topics) => {
    ...
    if (
      !isETH(token0) &&
      !supportedTokens.includes(token0) &&
      !supportedTokens.includes(token1)
    ) {
      return acc
    }
  ...
  const reserves = (
    await sdk.api.abi.multiCall({
      abi: 'erc20:balanceOf',
      calls: Object.keys(pairs)
        .map((pairAddress) => {
          const calls = []

          if (pairs[pairAddress].token0Address) {
            calls.push({
              target: isETH(pairs[pairAddress].token0Address)
                ? ETH_BALANCE
  ...
  return reserves.reduce((accumulator, res) => {
    ...
    const existingBalance = new BigNumber(accumulator[token] || '0')

    accumulator[token] = existingBalance.plus(balance).toFixed()
