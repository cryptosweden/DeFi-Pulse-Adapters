...
const COMPOUND_COMPTROLLER_ADDRESS =
...
async function getMarkets (block, liquidityPool) {
  if (block < 11908288) {
    ...
    const allTokens = await getAllTokens(block, liquidityPool)
    // if not in cache, get from the blockchain
    for (token of allTokens) {
      const kToken = await getKToken(block, token, liquidityPool)

      if (!markets[token]) {
        const info = await sdk.api.erc20.info(token)
        markets[token] = {
          kToken,
...
async function getHidingVaultBalances (timestamp, block) {
  ...
  if (block > HIDING_VAULT_START_BLOCK_NUMBER) {
    ...
    totalHidingVaultNFTs = totalHidingVaultNFTs
      .filter((hidingVaultNFT) => hidingVaultNFT.success == true)
    ...
    await Promise.all(
      cTokens.map(async (cTokenAddress) => {
        try {
          ...
          const sumTotal = underlyingBalances.output
            .map(({ output }) => output)
            .reduce((acc, val) => {
              return new BigNumber(acc).plus(new BigNumber(val)).toString(10)
            }, '0')

          hidingVaultBalances[token] = new BigNumber(
            hidingVaultBalances[token] || '0'
          )
            .plus(new BigNumber(sumTotal))
            .toString(10)
        } catch (err) {
          console.error(err)
        }
...
async function getLiquidityPoolBalances (timestamp, block) {
  ...
  for (const liquidityPool of Object.values(LIQUIDITY_POOL_CONTRACTS)) {
    const markets = await getMarkets(block, liquidityPool)
    // Get token balances
    ...
    const ethBalance = (
      await sdk.api.eth.getBalance({ target: liquidityPool, block })
    ).output
    liquidityPoolBalances[ETH] = BigNumber(liquidityPoolBalances[ETH] || 0)
      .plus(ethBalance)
      .toFixed()
  }
...
async function tvl (timestamp, block) {
  const liquidityPoolBalances = await getLiquidityPoolBalances(
    timestamp,
    block
  )
  ...
  _.uniq(
    Object.keys(hidingVaultBalances).concat(Object.keys(liquidityPoolBalances))
  ).forEach((asset) => {
    totalBalances[asset] = new BigNumber(hidingVaultBalances[asset] || '0')
      .plus(new BigNumber(liquidityPoolBalances[asset] || '0'))
      .toString(10)
  })
