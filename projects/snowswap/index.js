...
async function tvl (timestamp, block) {
  const blocks = [10867618, 11409366, 11662074, 12089837]
  const Pools = {
  ...
  for (const [index, [key, value]] of Object.entries(Object.entries(Pools))) {
    if (index == 0 || index == 1) {
      ...
      for (const coin in Pools[key]) {
        ...
        const p = await getYprice(block, coin, Pools[key][coin])
      ...
      for (const coin in Pools[key]) {
        ...
        if (coin == '0x7Ff566E1d69DEfF32a7b244aE7276b9f90e9D0f6') {
          ...
          // harvest btc
          // as crvRenBtc;
          const underlyingBal = await sdk.api.abi.call({
            block,
            target: coin,
