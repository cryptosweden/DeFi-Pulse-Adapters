...
const newLoopringExchangeDepositContract =
...
async function tvl (timestamp, block) {
  const ethBalanceOld = await sdk.api.eth.getBalance({
    target: oldLoopringExchange,
    block
  })
  const ethBalanceNew = await sdk.api.eth.getBalance({
    target: newLoopringExchangeDepositContract,
    block
  })
  const ethBlanaceTotal = BigNumber(ethBalanceOld.output || 0).plus(
    ethBalanceNew.output
  )
