...
const bzrxTokenAddress = '0x56d811088235F11C8920698a204A5010a788f4b3'
const vbzrxTokenAddress = '0xB72B31907C1C95F3650b64b2469e08EdACeE5e8F'
const bzxLetacyProtocolAddress = '0x8b3d70d628ebd30d4a2ea82db95ba2e906c71633'
const bzxProtocolAddress = '0xd8ee69652e4e4838f2531732a46d1f7f584f0b7f'
// TODO for future staking
const bzxStakingAddress = '0x576773CD0B51294997Ec4E4ff96c93d5E3AE9038'

const legacyRegistryContractAddress =
  '0xD8dc30d298CCf40042991cB4B96A540d8aFFE73a'
const registryContractAddress = '0xf0E474592B455579Fe580D610b846BdBb529C6F7'

const mkrAddress = '0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2'

const v2DeployBlock = 10829970
const v2VestingDeployBlock = 10441197
/* ==================================================
  Main
  ================================================== */
...
async function tvl (timestamp, block) {
  ...
  _.each(iTokens, (iToken) => {
    const supply = _.find(
      supplyResult.output,
      (result) => result.input.target === iToken.iTokenAddress
    )
    const borrow = _.find(
      borrowResult.output,
      (result) => result.input.target === iToken.iTokenAddress
    ...
    if (supply.success && borrow.success) {
      ...
      balances[iToken.underlyingAddress.toUpperCase()] = BigNumber(totalSupply)
        .minus(totalBorrow)
        .toFixed()
    }
  })
  ...
  function sumMultiBalanceOf (balances, results) {
    _.each(results.output, (result) => {
      if (result.success) {
        ...
        balances[address.toUpperCase()] = BigNumber(
          balances[address.toUpperCase()] || 0
        )
          .plus(balance)
          .toFixed()
      }
...
async function rates (timestamp, block) {
  ...
  _.each(iTokensNew, (iToken) => {
    const supply = _.find(
      supplyInterestRate.output,
      (result) => result.input.target === iToken.iTokenAddress
    )
    const borrow = _.find(
      borrowInterestRate.output,
      (result) => result.input.target === iToken.iTokenAddress
    )
    const totalBorrow = _.find(
      totalAssetBorrow.output,
      (result) => result.input.target === iToken.iTokenAddress
    )
    const symbol = _.find(
      underlyingSymbol.output,
      (result) => result.input.target === iToken.underlyingAddress
    )
