  ...
  if (block < 10271924) {
    ...
    await Promise.all(
      allCTokens.map(async (cToken) => {
        ...
        if (!foundMarket) {
          const underlying = await getUnderlying(block, cToken)
...
async function tvl (timestamp, block) {
  ...
  const v1Locked = await sdk.api.abi.multiCall({
    block,
  ...
  const v2Locked = await sdk.api.abi.multiCall({
    block,
    calls: _.map(markets, (market) => ({
  ...
  _.each(markets, (market) => {
    const getCash = _.find(
      v2Locked.output,
      (result) => result.success && result.input.target === market.cToken
    )
