...
async function tvl (timestamp, block) {
  ...
  const underlyingBalanceCalls = []
  const underlyingAddressCalls = []
  _.filter(poolBalances, (poolBalance) =>
    isCrToken(poolBalance.input.target)
  ).forEach((pooBalance) => {
  ...
  let [underlyingBalances, underlyingAddress, yVaultPrices, yCrvPrice] =
    await Promise.all([
  ...
  _.each(underlyingBalances, (underlying, i) => {
    if (underlying.success) {
      const balance = underlying.output
      const address = underlyingAddress[i].output
      const cAddress = underlying.input.target
  ...
  _.each(poolBalances, (balanceOf, i) => {
    if (balanceOf.success) {
      ...
      if (address === CRETH2) {
        balances[wETH] = BigNumber(balances[wETH] || 0)
          .plus(balances[CRETH2])
          .toFixed()
        delete balances[CRETH2]
      } else if (address === yyCrv) {
        const yyCrvCash = BigNumber(balance)
          .multipliedBy(yCrvPrice)
          .div(1e18)
          .div(1e12)
          .multipliedBy(yVaultPrices[1].output)
          .div(1e18)
          .integerValue()
