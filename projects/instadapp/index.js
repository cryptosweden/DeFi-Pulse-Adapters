...
async function tvl (timestamp, block) {
  ...
  const dsaEvents = await sdk.api.util.getLogs({
  ...
  const dsaWallets = _.map(dsaEvents.output, (event) => {
    return `0x${event[2].slice(26)}`
  })

  const allWallets = smartWallets.concat(dsaWallets)

  return (
    await sdk.api.cdp.getAssetsLocked({
      block,
      targets: allWallets
    })
  ).output
