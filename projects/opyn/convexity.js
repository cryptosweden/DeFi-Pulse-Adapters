...
module.exports = async function tvl (timestamp, block) {
  ...
  for (let i = 0; i < factoriesAddresses.length; i++) {
    ...
    const numberOfOptionsContracts = (
    ...
    const getOptionsContractsCalls = []
    ...
    _.each(optionsContracts, async (contracts) => {
      if (contracts.output != null) {
        optionsAddresses = [...optionsAddresses, contracts.output]
      }
    })

    // batch getCollateralAsset calls
    const getCollateralAssetCalls = []
    ...
    _.each(optionsCollateral, async (collateralAsset) => {
      ...
      if (
        collateralAsset.output.toLowerCase() != null &&
        collateralAsset.output.toLowerCase() !==
          '0x0000000000000000000000000000000000000000' &&
        supportedTokens.includes(collateralAsset.output.toLowerCase()) &&
        !optionsCollateralAddresses.includes(
          collateralAsset.output.toLowerCase()
        )
      ) {
    ...
    _.each(optionsAddresses, async (optionAddress) => {
      const balance = (
        await sdk.api.eth.getBalance({ target: optionAddress, block })
      ).output
