  ...
  if (block >= START_BLOCK) {
    const supportedTokens = await sdk.api.util
      ...
      .then((supportedTokens) =>
        supportedTokens.map(({ contract }) => contract)
      )

    // get ETH balance
    const balance = (
      await sdk.api.eth.getBalance({ target: marginPool, block })
    ).output
    balances[ETH] = BigNumber(balances[ETH] || 0)
    ...
    await Promise.all(
      supportedTokens.map(async (pulseToken) => {
        ...
        if (collateralAsset.substring(0, 2) == '0x' && collateralAsset != ETH) {
          const isWhitelistedCollateralToken = (
          ...
          if (isWhitelistedCollateralToken) {
            ...
            balances[collateralAsset] = BigNumber(
              balances[collateralAsset] || 0
            )
