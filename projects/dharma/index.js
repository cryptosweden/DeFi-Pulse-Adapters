...
async function tvl (timestamp, block) {
  ...
  if (block < 9518500) {
    ...
    const walletCalls = _.map(wallets, (wallet) => {
      return {
        target: wallet
      }
    })
    ...
    _.each(walletBalances, (walletBalance) => {
      if (walletBalance.success) {
        _.each(walletTokens, (walletToken) => {
          const address = walletToken.address

          _.each(walletToken.keys, (key) => {
            const balance = walletBalance.output[key]
    ...
    const dharmaDaiAddress = '0x00000000001876eB1444c986fD502e618c587430'
    const daiAddress = '0x6B175474E89094C44Da98b954EedeAC495271d0F'
    const dharmaUSDCAddress = '0x00000000008943c65cAf789FFFCF953bE156f6f8'
    const usdcAddress = '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48'
    const totalSupplyUnderlyingABI = {
      constant: true,
      inputs: [],
    ...
    const dharmaDaiUnderlyingBalance = (
    ...
    const dharmaDaiUnderlyingBalanceBN = BigNumber(
    ...
    const dharmaUSDCUnderlyingBalance = (
      await sdk.api.abi.call({
        block,
    ...
    const dharmaUSDCUnderlyingBalanceBN = BigNumber(
