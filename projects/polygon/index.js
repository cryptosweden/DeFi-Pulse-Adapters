...
async function tvl (_, block) {
  ...
  const balances = {
    [etherAddress]: (
      await sdk.api.eth.getBalance({ target: posEtherPredicate, block })
    ).output
  }
  ...
  const wrappedETHIndex = lockedPlasmaBalances.output.findIndex(
    (v) =>
      v.success &&
      v.input.target == '0xa45b966996374E9e65ab991C6FE4Bfce3a56DDe8'
  )
  if (wrappedETHIndex > -1) {
    balances[etherAddress] = new BigNumber(balances[etherAddress]).plus(
      lockedPlasmaBalances.output[wrappedETHIndex].output
    )
