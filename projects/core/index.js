 ...
 * @returns {Promise<{ [String]: BigNumber }>}
 */
async function getTokenUnderlyingReserves (
  token,
  defaultReserve,
  _timestamp,
  block
...
async function tvl (timestamp, block) {
  const pairInfo = await getUniswapPairInfo(
    configs.pairAddresses,
    timestamp,
    block
  )
  const underlyingReserves = await Promise.all(
    pairInfo.map((info) => getPairUnderlyingReserves(info, timestamp, block))
  )
