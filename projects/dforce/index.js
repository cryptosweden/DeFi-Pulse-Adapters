...
/* ==================================================
    Synth Markets
    ================================================== */
const allSynthMarkets = [USX, EUX, xBTC, xETH, xTSLA, xAAPL, xAMZN, xCOIN]
...
async function tvl (timestamp, block) {
  ...
  const assetsCalls = _.map(usdxReservedTokens, (token) => ({
    target: token,
    params: usdxPool
  }))
  ...
  const assetsBalanceResults = await sdk.api.abi.multiCall({
    block,
    calls: assetsCalls,
    abi: 'erc20:balanceOf'
  })
  ...
  const synthMarketsCalls = _.map(allSynthMarkets, (synthMarket) => ({
    target: synthMarket
  }))

  const synthMarketsResults = await sdk.api.abi.multiCall({
    block,
  ...
  balances[USDC] = BigNumber(balances[USDC] || 0)
    .plus(usdxRemainingUSDC)
    .toFixed()

  await Promise.all(
    yieldMarkets.map(async (yieldMarket) => {
      try {
        const marketTVL = (
          await sdk.api.abi.call({
            block,
        ...
        balances[yieldUnderlyingTokens[yieldMarket]] = BigNumber(
          balances[yieldUnderlyingTokens[yieldMarket]] || 0
        )
          .plus(_balance)
          .toFixed()
      } catch (error) {
  ...
  const iTokenLocked = await sdk.api.abi.multiCall({
    block,
  ...
  _.each(iTokenLocked.output, (lockedDetails) => {
    const asset = lendingUnderlyingTokens[lockedDetails.input.target]
    balances[asset] = BigNumber(balances[asset] || 0)
      .plus(lockedDetails.output || 0)
      .toFixed()
  })
