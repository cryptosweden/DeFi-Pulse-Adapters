...
async function tvl (timestamp, block) {
  ...
  let poolLogs = await sdk.api.util
    .getLogs({
  ...
  const pools = _.map(poolLogs, (poolLog) => {
  ...
  const [{ output: poolBalances }, { output: tokensUnderlyings }] =
    await Promise.all([
      ...
      sdk.api.abi.multiCall({
        block,
        calls: poolCalls.map((c) => ({ target: c.target })),
        abi: abi.underlying
      })
    ])
  ...
  for (let i = 0; i < poolBalances.length; i++) {
    ...
    const underlying = _.find(
      tokensUnderlyings,
      (t) => t.input.target === tokenAddress
    )
