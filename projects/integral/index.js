...
async function getPairs (block) {
  ...
  const [token0Addresses, token1Addresses] = await getTokenAddresses(
    block,
    pairAddresses
  )
  const pairs = constructPairs(
    supportedTokens,
    token0Addresses,
    token1Addresses
  )
