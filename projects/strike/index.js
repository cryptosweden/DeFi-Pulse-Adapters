...
async function getMarkets (block) {
  const allSTokens = await getAllSTokens(block)
  // if not in cache, get from the blockchain
  await Promise.all(
    allSTokens.map(async (sToken) => {
      ...
      if (!foundMarket) {
        const underlying = await getUnderlying(block, sToken)
...
async function tvl (timestamp, block) {
  ...
  const locked = await sdk.api.abi.multiCall({
    block,
  ...
  _.each(markets, (market) => {
    const getCash = _.find(
      locked.output,
      (result) => result.success && result.input.target === market.sToken
    )
