...
async function _handleCurve (token, underlyingAmounts) {
  const crvBalance = await _getCurveVirtualPrice(token, underlyingAmounts)
...
async function _getTricryptoPrice (holdings) {
  const url = 'https://stats.curve.fi/raw-stats-crypto/tricrypto-1m.json'
  ...
  const usdBalance = BigNumber(data[0].balances[0])
    .div(10 ** 6)
    .multipliedBy(badgerHoldingsRatio)
  const wBTCBalance = BigNumber(data[0].balances[1])
    .div(10 ** 8)
    .multipliedBy(badgerHoldingsRatio)
  const wETHBalance = BigNumber(data[0].balances[2])
    .div(10 ** 18)
    .multipliedBy(badgerHoldingsRatio)
...
async function tvl (timestamp, block) {
  ...
  bTokenSupplies.output.forEach((bTokenSupply) => {
    if (bTokenSupply.success) {
      ...
      if (
        underlyingAmounts[underlyingAddress] &&
        underlyingAmounts[underlyingAddress].gt(0)
      ) {
  ...
  try {
    ...
    if (diggVaultBalance.success) {
      const diggPpfs = BigNumber(diggVaultBalance.output)
        .div(10 ** 9)
  ...
  balances[cvxCrv] = isNaN(underlyingAmounts[cvxCrv])
    ? 0
    : underlyingAmounts[cvxCrv]
  ...
  await Promise.all(
    platforms.sushi.map(async (token) => {
      const underlyingTokenBalance = await _getPairValue(
        SUSHI_SUBGRAPH,
        token,
      ...
      underlyingTokenBalance.forEach((pair) => {
        if (
          ...
          balances[Object.keys(pair)[0]].gt(0)
        ) {
  ...
  await Promise.all(
    platforms.tricrypto.map(async (token) => {
      const tricryptoBalances = await _getTricryptoPrice(
        underlyingAmounts[token]
      )
      console.log(
        'checking tricrypto',
        balances[usdc],
      ...
      balances[usdc] && balances[usdc].gt(0)
        ? (balances[usdc] = tricryptoBalances[0].plus(balances[usdc]))
        : (balances[usdc] = tricryptoBalances[0])
      balances[wbtc] && balances[wbtc].gt(0)
        ? (balances[wbtc] = tricryptoBalances[1].plus(balances[wbtc]))
        : (balances[wbtc] = tricryptoBalances[1])
      balances[wETH] && balances[wETH].gt(0)
        ? (balances[wETH] = tricryptoBalances[2].plus(balances[wETH]))
        : (balances[wETH] = tricryptoBalances[2])
    })
  )
