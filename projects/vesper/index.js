...
async function tvl (timestamp, block) {
  ...
  // Get pool list
  const poolAddressListResponse = await sdk.api.abi.call({
    target: controller,
    abi: abi.pools
  })

  const poolsCountResponse = await sdk.api.abi.call({
  ...
  _.each(totalValueResponse.output, (response) => {
    if (response.success) {
      ...
      const existingBalance = new BigNumber(
        balances[collateralToken[poolAddress]] || '0'
      )
      balances[collateralToken[poolAddress]] = existingBalance
        .plus(new BigNumber(totalValue))
        .toFixed()
    }
  })
  return balances
