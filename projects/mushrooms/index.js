...
async function tvl (timestamp, block) {
  ...
  const addressListResponse = await axios.get(
    'https://vjeieiw4tf.execute-api.us-east-1.amazonaws.com/apy'
  )

  const vaultsInfo = addressListResponse.data.result.vaults

  const res = await Promise.all(
    vaultsInfo.map((info) => getBalance(info, timestamp, block))
  )
  const balances = {}
