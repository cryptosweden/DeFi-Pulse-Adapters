...
const rariGovernanceTokenUniswapDistributorAddress =
...
async function tvl (timestamp, block) {
  ...
  try {
    const ethPoolData = (
      await sdk.api.abi.multiCall({
        ...
        calls: earnETHPoolFundControllerAddressesIncludingLegacy.map(
          (address) => ({
            target: address
          })
        )
  ...
  // Earn DAI pool
  try {
    await getBalancesFromEarnPool(
      earnDAIPoolControllerAddressesIncludingLegacy
    )
  } catch (e) {
    // ignore error
  }
  ...
  try {
    ...
    const fractionStaked = BigNumber(totalStaked.output).dividedBy(
      BigNumber(lpTokenSupply.output)
    )
    ...
    if (
      rgtETHPairData &&
      rgtETHPairData.output &&
      fractionStaked.isGreaterThan(bigNumZero)
    ) {
      ...
      if (reserve0Balanace.isGreaterThan(bigNumZero)) {
        updateBalance(
          WETHTokenAddress,
          BigNumber(rgtETHPairData.output._reserve0).multipliedBy(
            fractionStaked
          )
        )
      }
      if (reserve1Balance.isGreaterThan(bigNumZero)) {
        updateBalance(
          RGTTokenAddress,
          BigNumber(rgtETHPairData.output._reserve1).multipliedBy(
            fractionStaked
          )
        )
      }
