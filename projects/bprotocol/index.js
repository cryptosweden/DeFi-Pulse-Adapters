  ...
  await Promise.all(
    cTokens.map(async (cTokenAddress) => {
      try {
        ...
        balances[token] = new BigNumber(balances[token] || '0')
...
async function makerTvl (timestamp, block) {
  ...
  ethBalances.forEach(
    (balance) => (totalBalance = totalBalance.plus(new BigNumber(balance)))
  )
  const balances = {
    '0x0000000000000000000000000000000000000000': totalBalance.toString(10)
  }
...
async function liquityTvl (timestamp, block) {
  if (block < lFirstBlock) {
    return { '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0': '0' }
  }
  ...
  // all balance is lusd
  return {
    '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0': totalBalance.toString(10)
  }
}

async function tvl (timestamp, block) {
  const [cTvl, mTvl, lTvl] = await Promise.all([
    compoundTvl(timestamp, block),
    makerTvl(timestamp, block),
    liquityTvl(timestamp, block)
  ])
  // combine balances for Maker and Compound B.Protocol's TVL
  const allLendingPlatformBalances = {}
  // all assets in B.Protocol
  _.uniq(
    Object.keys(cTvl).concat(Object.keys(mTvl)).concat(Object.keys(lTvl))
  ).forEach((asset) => {
    allLendingPlatformBalances[asset] = new BigNumber(cTvl[asset] || '0')
      .plus(new BigNumber(mTvl[asset] || '0'))
      .plus(new BigNumber(lTvl[asset] || '0'))
      .toString(10)
  })
