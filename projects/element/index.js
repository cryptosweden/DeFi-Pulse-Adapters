...
async function tvl (timestamp, block) {
  ...
  for (const log of trancheLogs) {
    if (block < log.blockNumber) continue
    const tranche = `0x${log.topics[1].substr(-40)}`
    ...
    balances[underlying.toLowerCase()] = balances[underlying.toLowerCase()]
      ? new bn(balances[underlying.toLowerCase()]).plus(valueSupplied)
      : valueSupplied
  }
  ...
  for (const wp of wps) {
    try {
      ...
      for (let i = 0; i < poolTokens.tokens.length; i++) {
        const token = poolTokens.tokens[i]
        let tranche
        try {
          tranche = (
            await sdk.api.abi.call({
        ...
        balances[token.toLowerCase()] = balances[token.toLowerCase()]
          ? new bn(balances[token.toLowerCase()]).plus(poolTokens.balances[i])
  ...
  for (const log of ccLogs) {
    if (block < log.blockNumber) continue
    const cc = `0x${log.topics[1].substr(-40)}`
    const poolId = (
      await sdk.api.abi.call({
        block,
        target: cc,
