...
async function tvl (timestamp, block) {
  ...
  for (const standaloneHypervisor of standaloneHypervisors) {
    if (
      block >= standaloneHypervisor.fromBlock &&
      block <= standaloneHypervisor.toBlock
    ) {
  ...
  const hypervisorCalls = []

  for (const hypervisor of Object.keys(hypervisors)) {
  ...
  for (const balance of hypervisorBalances.output) {
    if (balance.success) {
      const hypervisorAddress = balance.input.target
      const address0 = hypervisors[hypervisorAddress].token0Address
      const address1 = hypervisors[hypervisorAddress].token1Address
      const balance0 = balance.output.total0
      const balance1 = balance.output.total1

      balances[address0] = BigNumber(balances[address0] || 0)
        .plus(balance0)
        .toFixed()
      balances[address1] = BigNumber(balances[address1] || 0)
        .plus(balance1)
        .toFixed()
    }
  }
