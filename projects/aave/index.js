...
async function _multiMarketV1Rates (lendingPool, reserves, block) {
  ...
  const ratesData = { lend: {}, borrow: {}, supply: {}, borrow_stable: {} }
  ...
  Object.keys(uniswapMarketTvlBalances).forEach((address) => {
    if (balances[address]) {
      balances[address] = BigNumber(balances[address])
        .plus(uniswapMarketTvlBalances[address])
        .toFixed()
    } else {
...
async function rates (timestamp, block) {
  ...
  const ratesV1 = await _multiMarketV1Rates(
    aaveLendingPool,
    aaveReserves,
    block
  )
