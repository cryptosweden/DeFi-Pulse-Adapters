...
async function getJoins (block) {
  const rely = utils.sha3('rely(address)').substr(0, 10)
  const relyTopic = utils.padRight(rely, 64)
  const joins = {}
  ...
  await Promise.all(
    ilks.map(async (ilk) => {
      try {
        ...
        const name = utils.hexToString(ilk.output)
...
async function tvl (timestamp, block) {
  ...
  if (block >= MakerMCDConstants.STARTBLOCK) {
    const joins = await getJoins(block)

    await Promise.all(
      Object.keys(joins).map(async (join) => {
        try {
          const gem = (
            await sdk.api.abi.call({
              block,
          ...
          balances[gem] = balances[gem]
            ? balances[gem].plus(balance)
    ...
    balances[MakerMCDConstants.DAI] = balances[MakerMCDConstants.DAI]
      ? balances[MakerMCDConstants.DAI].plus(pie)
      : new BigNumber(pie)
  }
