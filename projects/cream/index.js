...
async function getMarkets (block) {
  ...
  const [underlyingDecimals, underlyingSymbols] = await Promise.all([
  ...
  _.each(allUnderlying, (underlying) => {
    let decimals = _.find(
      underlyingDecimals.output,
      (decimals) =>
        decimals.success && decimals.input.target === underlying.output
    )
    let symbol = _.find(
      underlyingSymbols.output,
      (symbol) => symbol.success && symbol.input.target === underlying.output
    )
    if (symbol && decimals) {
...
async function tvl (timestamp, block) {
  ...
  _.each(cTokens, (cToken) => {
    let getCash = _.find(
      cashes,
      (result) => result.success && result.input.target === cToken
    )
    ...
    if (getCash && underlying) {
      ...
      if (underlying === yETH) {
        ...
        const yETHCash = BigNumber(getCash)
        ...
        const ethCash = BigNumber(balances[wETH] || 0)
        const creth2Cash = BigNumber(getCash)
        ...
        const yCrvCash = BigNumber(getCash)
        ...
        const usdcCash = BigNumber(balances[usdc] || 0)
        const yUSDCash = BigNumber(getCash)
...
async function rates (timestamp, block) {
  ...
  _.each(markets, (data, underlying) => {
    const supplyRate = _.find(
      supplyResults,
      (result) => result.success && result.input.target == data.cToken
    )
    const borrowRate = _.find(
      borrowResults,
      (result) => result.success && result.input.target == data.cToken
    )
    const totalBorrows = _.find(
      totalBorrowsResults,
      (result) => result.success && result.input.target == data.cToken
    )
