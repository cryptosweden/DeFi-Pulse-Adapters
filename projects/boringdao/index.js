...
async function tvl (timestamp, block) {
  ...
  const obtcTVL = BigNumber(price.output[0].output)
    .multipliedBy(total.output[0].output)
    .div(10 ** 18)
  const oltcTVL = BigNumber(price.output[1].output)
    .multipliedBy(total.output[1].output)
    .div(10 ** 18)

  let result = obtcTVL
    .plus(oltcTVL)
    .plus(tunnelTVl.output[0].output)
    .plus(tunnelTVl.output[1].output)
