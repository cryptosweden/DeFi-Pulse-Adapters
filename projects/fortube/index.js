...
async function getErc20Assets (block) {
  const erc20Assets = []
  const calls = []
  const length = await getCollateralMarketsLength(block)
  ...
  const erc20AssetResults = await sdk.api.abi.multiCall({
    block,
    calls: calls,
...
async function allUnderlyingMarkets (block) {
  const fTokens = await getAllMarkets(block)
  const erc20AssetsV2 = []
...
async function getUnderlyingInfo (ftoken, block) {
  ...
  const info = await sdk.api.erc20.info(underlying)
...
async function tvl (timestamp, block) {
  const balances = {}
  const erc20Assets = await getErc20Assets(block)
  ...
  if (block > V2BLOCK) {
    const ethBalance2 = (
      await sdk.api.eth.getBalance({ target: ForTubeV2, block })
    ).output
    balances[EthAddress] = BigNumber(balances[EthAddress])
      .plus(ethBalance2)
      .toFixed()

    const erc20AssetsV2 = await allUnderlyingMarkets(block)
    const balanceOfResultsV2 = await sdk.api.abi.multiCall({
      block,
...
async function rates (timestamp, block) {
  const ratesData = { lend: {}, borrow: {}, supply: {} }
  const allAssets = await getErc20Assets(block)
    ...
    mktsResults.map(async (market) => {
      if (market.success) {
        ...
        if (asset == '0x9f8F72aA9304c8B593d555F12eF6589cC3A579A2') {
          ...
          const info = await sdk.api.erc20.info(asset)
          symbol = info.output.symbol
          decimals = info.output.decimals
        }

        ratesData.lend[symbol] = String(
          (market.output.supplyRate / 1e18) * 100
        )
        ratesData.borrow[symbol] = String(
          (market.output.demondRate / 1e18) * 100
        )
        ratesData.supply[symbol] = BigNumber(market.output.totalBorrows)
          .div(10 ** decimals)
          .toFixed()
      }
    })
  ...
  // V2
  if (block > V2BLOCK) {
    const APRs = await multiGet('APR', block)
    const APYs = await multiGet('APY', block)
    const Supplies = await multiGet('totalBorrows', block)

    await Promise.all(
      APRs.map(async (APR) => {
        if (APR.success) {
          const info = await getUnderlyingInfo(APR.input.target, block)
    ...
    await Promise.all(
      APYs.map(async (APY) => {
        if (APY.success) {
          const info = await getUnderlyingInfo(APY.input.target, block)
    ...
    await Promise.all(
      Supplies.map(async (supply) => {
        if (supply.success) {
          const info = await getUnderlyingInfo(supply.input.target, block)
          ratesData.supply[info.symbol] = BigNumber(supply.output)
            .div(10 ** info.decimals)
            .toFixed()
        }
      })
    )
