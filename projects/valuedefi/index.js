...
async function singleVaultTvl (timestamp, block) {
  ...
  _.forEach(totalSupplies, (totalSupply, index) => {
    const balance = new BigNumber(totalSupply || 0)
    const asset = configs.vault.single[index].underlyingToken
    const total = new BigNumber(balances[asset] || 0)
...
async function uniSwapPairInfo (pairAddresses, timestamp, block) {
  const [token0Addresses, token1Addresses, reserves, totalSupplies] =
    await Promise.all([
      sdk.api.abi
        .multiCall({
          abi: token0,
...
async function mergeBalance (array) {
  ...
  for (const balances of array) {
    for (const asset in balances) {
      ...
      const total = new BigNumber(globalBalances[asset.toLowerCase()] || 0)
...
async function cuNIv2LPVaultTvl (timestamp, block) {
  ...
  const pairs = configs.vault.cVaultUniV2LP.map(
    (value) => value.underlyingToken
  )
  ...
  for (let index = 0; index < configs.vault.uniV2LP.length; index++) {
    const vaultSupply = new BigNumber(vaultBalance[index] || 0)
    const { reserve0, reserve1, token0, token1, totalSupply } =
      pairInfos[index]
    if (reserve0 && reserve1 && token0 && token1 && totalSupply) {
      balanceOfResult.push({
        address: token0,
        balance: new BigNumber(reserve0)
          .times(vaultSupply)
          .div(totalSupply)
          .toFixed()
      })
      balanceOfResult.push({
        address: token1,
        balance: new BigNumber(reserve1)
          .times(vaultSupply)
          .div(totalSupply)
          .toFixed()
      })
  ...
  _.forEach(balanceOfResult, (result) => {
    const balance = new BigNumber(result.balance || 0)
    const asset = result.address
    const total = new BigNumber(balances[asset] || 0)
    balances[asset] = balance.plus(total).toFixed()
  })
  return balances
...
async function cVaultSeedTvl (timestamp, block) {
  ...
  _.forEach(vaultBalances, (totalSupply, index) => {
    const balance = new BigNumber(totalSupply || 0)
    const asset = configs.vault.cVaultSeed[index].underlyingToken
    const total = new BigNumber(balances[asset] || 0)
    balances[asset] = balance.plus(total).toFixed()
  })
  return balances
...
async function cVaultBalancerTvl (timestamp, block) {
  ...
  const pools = configs.vault.cVaultBalancer.map(
    (value) => value.underlyingToken
  )
  const poolCalls = []
  const poolTokenData = (
  ...
  _.each(poolBalances, (balanceOf, index) => {
    if (balanceOf.success) {
      ...
      const vaultBalance = poolCalls[index].vaultBalance || 0
      const totalSupply = poolCalls[index].totalSupply || 0
...
async function uNIv2LPVaultTvl (timestamp, block) {
  ...
  const balanceOfResult = []
  for (let index = 0; index < configs.vault.uniV2LP.length; index++) {
    const vaultSupply = new BigNumber(totalVaultSupplies[index] || 0)
    const { reserve0, reserve1, token0, token1, totalSupply } =
      pairInfos[index]
    if (reserve0 && reserve1 && token0 && token1 && totalSupply) {
      balanceOfResult.push({
