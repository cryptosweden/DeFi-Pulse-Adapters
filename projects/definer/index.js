...
const utility = {
  ...
  async getMarkets (block) {
    ...
    const tokenRegistryAddress =
      await utility.getTokenRegistryAddressByGlobalConfig(block)

    // Get latest markets
    const currentMarkets = await utility.getTokenRegistryContract(
      block,
  ...
  async getBankPoolAmounts (block, markets) {
    const bankAddress = await utility.getBankAddressByGlobalConfig(block)
    const callsArray = []
  ...
  async getCtokenValue (block, ctoken) {
    const cEthToken = await sdk.api.abi.call({
      target: ctoken,
  ...
  async getCTokens (block, markets) {
    ...
    const zeroCTokenAddress = '0x0000000000000000000000000000000000000000'
    cToken.forEach((item) => {
      if (item.success) {
  ...
  async handlerTokenApr (block, markets) {
    ...
    const yearTotalBlocksNumber = 365 * 24 * 60 * 4

    const targetTokenObj = {}

    const allTokenObj = await utility.getCTokens(block, markets)

    const ctokenMapToken = {}
    const callsCompArray = []
    const capitalCompoundRatioArray = []
    ...
    const tempValue = BigNumber(3).times(Math.pow(10, 16)).toFixed(0)
    capitalUtilizationRatio.forEach((item) => {
      ...
      if (item.success) {
        ...
        const notSupportCompBorrowRatePerBlock = BigNumber(item.output)
    ...
    Object.keys(targetTokenObj).forEach((token_address) => {
      const item = targetTokenObj[token_address]
      let borrowRatePerBlock
      let depositRatePerBlock
      if (item.ctoken) {
        const supplyRatePerBlockCompValue = BigNumber(
          item.supplyRatePerBlockComp
        )
          .times(0.4)
          .toFixed(0)
        const borrowRatePerBlockCompValue = BigNumber(
          item.borrowRatePerBlockComp
        )
          .times(0.6)
          .toFixed(0)
  ...
  formatRates (tokensObj) {
    ...
    Object.keys(tokensObj).forEach((tokenAddress) => {
      const tokenItem = tokensObj[tokenAddress]
...
async function tvl (timestamp, block) {
  ...
  if (block > 10819469) {
    ...
    const markets = await utility.getMarkets(block)

    // Get Bank
    const banksPoolAmounts = await utility.getBankPoolAmounts(block, markets)
    ...
    // cETH value
    balances['0x4Ddc2D193948926D02f9B1fE9e1daa0718270ED5'] =
      await utility.getCtokenValue(
        block,
...
async function rates (timestamp, block) {
  ...
  if (block > 10819469) {
    ...
    const tokenSymbols = await utility.getSymbol(block, markets)
    ...
    // handle token supply
    const banksContractTokenState = await utility.getBankContractTokenState(
      block,
      markets
    )
    ...
    // handle token APR
    const tokenAprInfoObj = await utility.handlerTokenApr(block, markets)
  ...
  const result = utility.formatRates(initTokens)
  return result
}
