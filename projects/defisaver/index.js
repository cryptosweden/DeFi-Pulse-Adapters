...
async function tvl (timestamp, block) {
    ...
    makerSubs.forEach((cdp, i) => {
      cdpInfo = [
        ...cdpInfo,
        {
          id: cdp[5],
          ilk: bytesToString(
            cdpsDetailed.find(({ input }) => input.params[0] === cdp[5])
              .output[3]
          )
        }
      ]
  ...
  if (block >= getContractBlock('CompoundSubscriptions')) {
    compoundSubs = (
      await sdk.api.abi.call({
        block,
  ...
  if (block >= getContractBlock('AaveSubscriptions')) {
    aaveSubs = (
      await sdk.api.abi.call({
        block,
  ...
  for (const sub of compoundSubs) {
    compoundSubscribers.add(sub[0])
  }
  for (const sub of aaveSubs) {
    aaveSubscribers.add(sub[0])
  }

  const makerBalances = (
    await sdk.api.cdp.maker.getAssetsLocked({
      block,
      targets: cdpInfo,
      ids: true
    })
  ...
  const compoundBalances = (
    await sdk.api.cdp.compound.getAssetsLocked({
      block,
  ...
  const aaveBalances = (
    await sdk.api.cdp.aave.getAssetsLocked({
      block,
