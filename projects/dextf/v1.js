...
const cTokensMap = {
  '0x4Ddc2D193948926D02f9B1fE9e1daa0718270ED5':
    '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', // cETH: WETH
  '0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643':
    '0x6B175474E89094C44Da98b954EedeAC495271d0F', // cDAI: DAI
  '0xC11b1268C1A384e55C48c2391d8d480264A3A7F4':
    '0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599' // cWBTC: WBTC
}
...
module.exports = async function tvl (timestamp, block) {
  ...
  const tokensList = []
  const response = await axios.get('https://api.dextf.com/dp/assets', {})
  ...
  const cTokenConversionRatesMap = (
  ...
  _.each(balanceOfResults.output, (balanceOf) => {
    if (balanceOf.success) {
      ...
      if (address in cTokensMap) {
        const addressOfUnderlying = cTokensMap[address]
        const conversionRate = BigNumber(cTokenConversionRatesMap[address])
        const balanceOfUnderlying = BigNumber(balanceOf.output)
          .times(conversionRate)
          .div(cTokenDecimalScale)

        balances[addressOfUnderlying] = BigNumber(
          balances[addressOfUnderlying] || 0
        )
          .plus(balanceOfUnderlying)
          .toFixed()
      } else {
