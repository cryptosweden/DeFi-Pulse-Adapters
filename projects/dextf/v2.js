...
module.exports = async function tvl (timestamp, block) {
  ...
  _.each(positionsForSets, function (positionForSet, i) {
    ...
    _.each(positionForSet.output, (position) => {
      ...
      if (isExternalPosition) {
        uniswapPositions[componentAddress] = BigNumber(
          uniswapPositions[componentAddress] || 0
        )
          .plus(positionUnits.times(setSupply))
          .toFixed()
      } else {
        balances[componentAddress] = BigNumber(balances[componentAddress] || 0)
          .plus(positionUnits.times(setSupply).div(SUPPLY_SCALE))
          .toFixed()
      }
    })
  })
  ...
  _.each(reserves, function (reserve, i) {
    ...
    const setSupplyRatio = new BigNumber(uniswapPositions[pairAddress])
      .div(new BigNumber(reserveSupplies[i].output))
      .div(SUPPLY_SCALE)
    ...
    if (tokenPair[0]) {
      ...
      if (!reserve0.isZero()) {
        const existingBalance = new BigNumber(balances[tokenPair[0]] || '0')
    ...
    if (tokenPair[1]) {
      ...
      if (!reserve1.isZero()) {
        const existingBalance = new BigNumber(balances[tokenPair[1]] || '0')
