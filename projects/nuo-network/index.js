...
async function tvl (timestamp, block) {
  ...
  const lastReserveRunsResults = await sdk.api.abi.multiCall({
    block,
  ...
  const reservesResults = await sdk.api.abi.multiCall({
    ...
    calls: _.map(lastReserveRunsResults.output, (lastReserveRuns) => {
      const lastUpdate = Number(lastReserveRuns.output)
      return {
        target: lastReserveRuns.input.target,
        params: [
          String(dayTimestamp > lastUpdate ? lastUpdate : dayTimestamp),
          lastReserveRuns.input.params[0]
        ]
      }
    }),
    abi: abi.reserves
  ...
  _.each(reservesResults.output, (reserves) => {
    if (reserves.success) {
      const address = reserves.input.params[1]
      balances[address] = BigNumber(balances[address] || 0)
        .plus(reserves.output)
        .toFixed()
    }
  })

  const accounts = await sdk.api.abi.call({
...
async function rates (timestamp, block) {
  ...
  const reserves = (
    await axios.get(
      'https://api.nuoscan.io/overview?primary_currency_short_name=USD'
    )
  ).data.data.reserves
