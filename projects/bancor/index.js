...
async function generateCallsByAPI (timestamp) {
  ...
  let moreData = true
  let index = 0
  const pageFetchCount = 300

  while (moreData) {
    let converters = await axios.get(
      'https://api.bancor.network/0.1/converters',
      {
        params: {
          skip: index,
    ...
    tokenConverters = [...tokenConverters, ...converters]
  ...
  tokenConverters = _.filter(tokenConverters, (converter) => {
    const hasLength = converter.details.length > 0
    const isEthereum = converter.details[0].blockchain.type === 'ethereum'
    const createdTimestamp = moment(converter.createdAt).utcOffset(0).unix()
    const existsAtTimestamp = createdTimestamp <= timestamp
  ...
  _.each(tokenConverters, (converter) => {
    const details = converter.details[0]
    const reserves = details.reserves

    const owners = _.map(converter.converters, (converter) => {
    ...
    _.each(owners, (owner) => {
      ...
      _.each(reserves, (reserve) => {
        const address = reserve.blockchainId
...
async function tvl (timestamp, block) {
  ...
  await Promise.all(
    ethBalanceCalls.map(async (call) => {
      const ethBalance = (
        await sdk.api.eth.getBalance({ target: call.params[0], block })
      ).output
  ...
  result.output = result.output.filter(
    (item) => item.success && item.output.length < 60
  )
