...
async function getSingleAssetVault (contractName, timestamp, block) {
  ...
  const [totalSupply, sharePrice, underlyingUnit] = await Promise.all([
    ...
    sdk.api.abi.call({
      block,
      target: vault.contract.address,
      abi: abi.fABISharePrice
    }),
    sdk.api.abi.call({
      block,
      target: vault.contract.address,
      abi: abi.fABIUnderlyingUnit
    })
  ])
  ...
  return [address, underlyingCount]
}
...
async function getLiquidityPool (contractName, timestamp, block) {
  ...
  const harvestShareOfPool = lpTokenCount.div(
    BigNumber(underlyingTotalSupply.output)
  )
  const amountToken0 = BigNumber(underlyingReserves.output[0]).times(
    harvestShareOfPool
  )
  const amountToken1 = BigNumber(underlyingReserves.output[1]).times(
    harvestShareOfPool
  )
...
async function getMooniswapLP (contractName, timestamp, block) {
  ...
  const [
    ...
    sdk.api.abi.call({
      block,
      target: underlyingAddress,
      abi: abi['1inchGetBalanceForAddition'],
  ...
  const amountToken0 = BigNumber(underlyingToken0.output).times(
    harvestShareOfPool
  )
  const amountToken1 = BigNumber(underlyingToken1.output).times(
    harvestShareOfPool
  )
...
async function getUniV3Vaults (vault, timestamp, block) {
  ...
  const [positions, poolLiquidity, uniBalanceToken0, uniBalanceToken1] =
    await Promise.all([
      positionsFn,
      poolLiquidityFn,
      uniBalanceToken0Fn,
  ...
  const liquidityShare = BigNumber(positions.output.liquidity).div(
    BigNumber(poolLiquidity.output)
  )
...
async function tvl (timestamp, block) {
  ...
  const uniV3Values = await Promise.all(
    Object.values(uniV3Vaults).map((vault) =>
      getUniV3Vaults(vault, timestamp, block)
    )
  )

  uniV3Values.forEach((val) => {
    for (const [k, v] of Object.entries(val)) {
      const existingVal = BigNumber(assetAmounts[k] || 0)
  ...
  const mooniswapVaultValues = await Promise.all(
    oldMooniswapVaults.map((contractName) =>
      getMooniswapLP(contractName, timestamp, block)
    )
  )
  ...
  const singleAssetVaultValues = await Promise.all(
    singleAssetVaults.map((contractName) =>
      getSingleAssetVault(contractName, timestamp, block)
    )
  )

  singleAssetVaultValues.forEach((val) => {
    const existingVal = BigNumber(assetAmounts[val[0]] || 0)
    assetAmounts[val[0]] = BigNumber.sum(existingVal, val[1])
  })

  /// ///////////  Liquidity pools ///////////////////////////
  const liquidityPoolValues = await Promise.all(
    liquidityPools.map((contractName) =>
      getLiquidityPool(contractName, timestamp, block)
    )
  )
