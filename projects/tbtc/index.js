...
async function tvl (timestamp, block) {
  ...
  const totalETHBonded = sdk.api.eth.getBalance({
    target: bondingContract,
    block
  })
  ...
  const totalKEEPStaked = BigNumber((await totalKEEPBonded).output)
    .plus((await KeepStakedVault).output)
    .toFixed()
