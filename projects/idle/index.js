...
async function tvl (timestamp, block) {
  ...
  Object.keys(contracts).forEach((contractName) => {
    const call = new Promise(async (resolve, reject) => {
      ...
      const tokenDecimals =
        underlyingTokens[contractInfo.underlyingToken].decimals
      const underlyingTokenAddr =
        underlyingTokens[contractInfo.underlyingToken].address

      let [tokenPrice, compBalance, totalSupply, underlyingTokenBalance] =
        await Promise.all([
