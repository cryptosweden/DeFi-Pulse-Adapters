...
const tokenAddresses = [WETH_ADDRESS, SAI_ADDRESS, DAI_ADDRESS, USDC_ADDRESS]

const allOpportunities = [
  {
    address: '0xEa5ee32F3A63c3FaBb311c6E8c985D308A53dcC1',
    beginTimestamp: 1568057588,
    endTimestamp: null,
    coins: [SAI_ADDRESS, DAI_ADDRESS, USDC_ADDRESS, WETH_ADDRESS]
  },
  {
    address: '0xEB6394f817b498c423C44bD72c3D7f8ED5DeeC6e',
    beginTimestamp: 1568057609,
    endTimestamp: null,
    coins: [SAI_ADDRESS, DAI_ADDRESS, USDC_ADDRESS, WETH_ADDRESS],
    added: [0, 1575650224, 1568341399, 0]
  },
  {
    address: '0x759A728653C4d0483D897DCCf3a343fe2bBbb54A',
    beginTimestamp: 1570466036,
    endTimestamp: null,
    coins: [SAI_ADDRESS, DAI_ADDRESS, USDC_ADDRESS, WETH_ADDRESS]
  },
  {
    address: '0xC830217BD3000E92CE846C549de6a2A36AEa8954',
    beginTimestamp: 1575649643,
    endTimestamp: null,
    coins: [DAI_ADDRESS]
  },
  {
    address: '0x3d6fa1331E142504Ba0B7965CD801c7F3b21b6C0',
    beginTimestamp: 1583869244,
    endTimestamp: null,
...
function getOpportunityCalls (timestamp) {
  ...
  for (let i = 0; i < allOpportunities.length; i++) {
    if (
      timestamp >= allOpportunities[i].beginTimestamp &&
      ...
      if (allOpportunities[i].added) {
        supportedTokens = removeTokens(
          timestamp,
          supportedTokens,
          allOpportunities[i].added
        )
      }
...
async function tvl (timestamp, block) {
  ...
  const { pmCalls, portfolioManagers } = getPmCalls(timestamp)
  const opportunityCalls = getOpportunityCalls(timestamp)

  await Promise.all(
    _.map(portfolioManagers, (portfolioManager) => {
      return new Promise(async (resolve, reject) => {
        try {
          const balance = (
            await sdk.api.eth.getBalance({ target: portfolioManager, block })
          ).output

          balances[ETH_ADDRESS] = BigNumber(balances[ETH_ADDRESS] || 0)
            .plus(balance)
            .toFixed()
          resolve()
        } catch (error) {
  ...
  _.each(opportunityBalances, (result) => {
    if (result.success) {
      const balance = result.output
      const address = result.input.params
