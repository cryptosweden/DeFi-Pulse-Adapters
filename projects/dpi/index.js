...
async function getBalances (block, target) {
  ...
  const calls = components.map((erc20) => ({
    target: erc20,
    params: target
  }))
  ...
  return componentBalances.reduce(
    (r, o) => Object.assign(r, { [o.input.target]: o.output }),
    {}
  )
}
...
async function tvl (timestamp, block) {
  ...
  const dpiBalances = await getBalances(block, assetContracts.dpi)
  ...
  const cgiBalances =
    block >= 11826294 || timestamp >= 1613028914
      ? await getBalances(block, assetContracts.cgi)
      : {}

  // Same as CGI, we need to start getting balances since the inception timestamp
  const fliBalances =
    block >= 12035541 || timestamp >= 1615709597
      ? await getBalances(block, assetContracts.fli)
      : {}

  // MVI inception timestamp
  const mviBalances =
    block >= 12184150 || timestamp >= 1617688800
      ? await getBalances(block, assetContracts.mvi)
      : {}

  // btc fli timestamp
  const btcBalances =
    block >= 12375760 || timestamp >= 1620238070
      ? await getBalances(block, assetContracts.flibtc)
      : {}
