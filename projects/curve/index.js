...
async function tvl (timestamp, block) {
  ...
  const registryStart = block > CurveRegistryStart
  const factoryStart = block > factoryStartBlock
  ...
  for (let i = 0; i < poolCount; i++) {
    const poolAddress = registryStart
      ? (
          await sdk.api.abi.call({
            block,
            target: CurveRegistryAddress,
