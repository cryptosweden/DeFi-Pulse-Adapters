...
const CSV = {
  ...
  create: async ({ output, slug }) => {
    ...
    await Promise.all(
      output.map(async (item, index) => {
        ...
        Object.keys(item.output).map((asset) => {
          if (fields.filter((field) => field.id === asset).length === 0) {
            // Add new field
            fields.push(
              {
                // Balance
    ...
    csvWriter.writeRecords(CSV.sortEntries(entries)).then(() => CSV.notify())
  },
  ...
  timestampToDate: (timestamp) => new Date(parseFloat(timestamp) * 1000),
  ...
  notify: () =>
    console.log(
      '** Verification CSV has been generated and is ready for your review in the /csv folder. **'
    ),
  // Get a historical price given a price and timestamp
  getPrice: ({ symbol, timestamp }) => {
    const url = `${process.env.DEFIPULSE_API_URL}/${process.env.DEFIPULSE_KEY}/price/ticker`
    const req = {
      method: 'POST',
      url,
      headers: {
