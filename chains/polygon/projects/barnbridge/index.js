...
/*==================================================
  Settings
==================================================*/
const SY_POOLS_API_URL =
  "https://prod-poly-v2.api.barnbridge.com/api/smartyield/pools";
const SA_POOLS_API_URL =
  "https://prod-poly-v2.api.barnbridge.com/api/smartalpha/pools";
...
async function tvl(timestamp, ethBlock) {
  ...
  await Promise.all(
    syPools.map(async (syPool) => {
      const { smartYieldAddress, underlyingAddress } = syPool;
      const underlyingTotal = await syGetUnderlyingTotal(
        chain,
        smartYieldAddress,
        block
      );
