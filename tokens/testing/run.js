  ...
  try {
    ...
    if (typeof timeUnit === 'number') {
      ...
      timestamp = moment()
        .utcOffset(0)
        .startOf(timeUnit)
        .add(timeOffset, timeUnit)
        .unix()
    }

    const point = await sdk.api.util.lookupBlock(timestamp)
    ...
    if (runFunction === 'balance') {
      ...
      output = (
        await sdk.api.util.unwrap({ balances: output, block: point.block })
      ).output
    }

    const ethCallCount = await sdk.api.util.getEthCallCount()
    return {
      ...ethCallCount,
      ...point,
      output
...
module.exports = (tokenAdapter, timeUnit, timeOffset = 0) => {
  ...
  for (const runFunction of functions) {
    if (typeof timeUnit === 'number') {
      label = `returns valid ${runFunction} data at ${moment
        .unix(timeUnit)
        .utcOffset(0)
        .format()}`
    } else {
    ...
    it(label, async function () {
      if (runFunction === 'balance') {
        ...
        _.each(projectRun.output, ({ balance: value }, symbol) => {
          const balance = BigNumber(value).toNumber()
